...
class FontSubsetter
{
	...
	public function makeSubsetSIP($file, array $subset, $TTCfontID = 0, $debug = false, $useOTL = 0)
	{
		...
		$cmap = [
			6, 0, 0, 1, // Format 6 Mapping subtable: format, length (set below), language, firstCode
		...
		$cmapstr6 = TableWriter::uint16s($cmap);
		$format6Length = strlen($cmapstr6);
		...
		if ($format6Length > 0xFFFF) {
			...
			$cmapstr = TableWriter::uint16s([0, 2, 1, 0]) . TableWriter::uint32(20)
				...
				. TableWriter::setUInt16($cmapstr6, 2, $format6Length)
				. $cmapstr4;
		}
