...
class Otl
{
	...
	function applyOTL($str, $useOTL)
	{
		...
		for ($sch = 0; $sch <= $subchunk; $sch++) {
			...
			$this->shaper = $this->shaperForScriptTag($scriptblock, $GSUBscriptTag);

			// A run the font offers no script for still goes to its shaper, which reorders it with
			// no lookups to apply. Every script with a shaper is one useOTL opens with 0x80, and a
			...
			$GSUBFeatures = $this->gsubFeatures($GSUBscriptTag, $GSUBlangsys);
	...
	private function applyGSUB($GSUBscriptTag, $GSUBlangsys, $GSUBFeatures, $scriptblock, $is_old_spec)
	{
		...
		$this->loadGsubData($GSUBscriptTag, $GSUBlangsys, (bool) $GSUBFeatures);
	 ...
	 */
	private function loadGsubData($GSUBscriptTag, $GSUBlangsys, $hasFeatures)
	{
		...
		// Nothing past this is read for a run with no lookups, and its font need not have a GSUB
		// table at all
		if (!$hasFeatures) {
			return;
		}
	 ...
	 */
	private function shaperForScriptTag($scriptblock, $GSUBscriptTag)
	{
		if ($this->shaper == 'K' || $this->shaper == 'T' || $this->shaper == 'L' || $scriptblock == Ucdn::SCRIPT_ARABIC) {
			return $this->shaper;
		}

		if ($scriptblock == Ucdn::SCRIPT_SYRIAC) {
			return $GSUBscriptTag == 'DFLT' ? '' : $this->shaper;
		}

		if ($GSUBscriptTag == 'DFLT' || $GSUBscriptTag == 'latn' || ($this->shaper == 'M' && $GSUBscriptTag == 'mymr')) {
			return '';
		}
	...
	/**
	 * @return array The GSUB features the font offers under a script and language system, by tag,
	 *               or none where it offers neither
	 */
	private function gsubFeatures($scriptTag, $langsys)
	{
		return isset($this->mpdf->CurrentFont['GSUBFeatures'][$scriptTag][$langsys])
			? $this->mpdf->CurrentFont['GSUBFeatures'][$scriptTag][$langsys]
			: [];
	}
	...
	function _applyGSUBrules($usetags, $scriptTag, $langsys)
	{
		...
		$GSUBFeatures = $this->gsubFeatures($scriptTag, $langsys);
		$LookupList = [];
