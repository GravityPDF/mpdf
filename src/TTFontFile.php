	...
	private function readScriptsAndFeatures($scriptListOffset, $featureListOffset)
	{
		...
		foreach ($scripts as $scriptTag => $languages) {
			foreach ($languages as $langTag => $featureIndices) {
				...
				// lookup 1 - and the group keeps them in the order the language system listed them, which
				// is the order that decides which of them Otl labels their shared lookup with. A usort()
				// over the features would settle that differently on PHP before 8.0, where sorts were not
				// stable, and write a font cache no other version agrees with.
				ksort($byFirstLookup);
